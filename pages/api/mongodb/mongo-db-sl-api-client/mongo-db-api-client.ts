...
export abstract class MongoDBApiClient extends NextApiClient {
  ...
  static async getLobbyData(lobby) {
    ...
    const response = await this.db.read(query).then((data) => data.resources[0]);
    return response
  }
  ...
  static async getUserData(user): Promise<UserLobbyData> {
    ...
    const userStartedMakingAListResponse = this.db.read(userStartedMakingAListQuery).then((data) => removeDuplicateObjectsInArray(data.resources));
    const lobbiesInvitedToQuery = `SELECT c.id, c.description FROM c JOIN t in c.invited_users WHERE t.to = '${user.user.email}'`;
    const lobbiesInvitedToResponse = this.db.read(lobbiesInvitedToQuery).then((data) => removeDuplicateObjectsInArray(data.resources));
    const response = await Promise.all([userResponse, userStartedMakingAListResponse, lobbiesInvitedToResponse])
    return {
