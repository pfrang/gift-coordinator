...
import FrontPageForm2 from './FrontPageForm2';


function FrontPageForm() {

  const [id, setID] = useState('');
  const [lobbyDescription, setLobbyDescription] = useState(false);
  ...
  const onSubmit = async (e) => {
    ...
    if (type === "Join") {
      ...
      try {
        ...
        setLobbyDescription(true);
        // router.push(`/lobby/${id}`)
      } catch (err) {
        console.error(err)
        errorTag.innerHTML = `A lobby with id ${id} already exists`
      }
    }
    else {
  ...
  return (
    <div className='flex justify-center items-center h-full'>
      {lobbyDescription ? <FrontPageForm2 id={id} lobbyDesc={setLobbyDescription}/> :
